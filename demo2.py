...
import __init_paths
from face_enhancement2 import FaceEnhancement #### yerang edit
...
if __name__=='__main__':
    ...
    dataloader = DataLoader(files, batch_size = 8, shuffle = False) 
    ...
    for n, data in enumerate(dataloader):
        ...
        #img_out, orig_faces, enhanced_faces = processer.process(imgs, aligned=args.aligned)
        img_outs, _, _ = processer.process(np.array(imgs), aligned=args.aligned)
        
        
        for i, file in enumerate(data):
            filename = os.path.basename(file)
            img = cv2.resize(imgs[i], img_outs[i].shape[:2][::-1])
            #cv2.imwrite(os.path.join(args.outdir, '.'.join(filename.split('.')[:-1])+f'_COMP{args.ext}'), np.hstack((img, img_out)))
            cv2.imwrite(os.path.join(args.outdir, '.'.join(filename.split('.')[:-1])+f'_GPEN{args.ext}'), img_outs[i])
