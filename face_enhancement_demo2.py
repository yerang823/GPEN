...
def faceEnhance(img, filename='output.jpg', outdir='sr_output', sr_scale=4, tile_size=32, verbose=False):
    ...
    faceenhancer = FaceEnhancement(in_size=model['size'], model=model['name'], use_sr=True, sr_model=model['sr_model'], sr_scale=model['sr_scale'], tile_size=tile_size, channel_multiplier=model['channel_multiplier'], narrow=model['narrow'])
...
def run_faceEnhance(target_fnames, outdir, sr_scale=4, tile_size=64, verbose=False):
    im_li = Path(target_fnames).glob('*')
    for target_fname in tqdm.tqdm(sorted(im_li)):

        
        tmp_name = outdir+'/'+target_fname.name.split('.')[0]+'_GPEN.jpg'
