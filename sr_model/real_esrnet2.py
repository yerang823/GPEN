...
class RealESRNet(object):
    ...
    def load_srmodel(self, base_dir, model):
        ...
        if torch.cuda.device_count() > 1: 
            self.srmodel = torch.nn.DataParallel(self.srmodel)
...
#             return None
    ...
    def process(self, imgs): ## 이미지 여러장 추론
        imgs = np.array(imgs)
        print('imgs.shape 11111 =======', imgs.shape)
        imgs = imgs.astype(np.float32) / 255.
        #img = torch.from_numpy(np.transpose(img[:, :, [2, 1, 0]], (2, 0, 1))).float()
        #img = img.unsqueeze(0).to(self.device)
        imgs = torch.from_numpy(np.transpose(imgs[:, :, :, [2, 1, 0]], (0,3,1,2))).float()
        print('imgs.shape 22222 =======', imgs.shape)
        ...
        if mod_scale is not None: # 일단 무시
            h_pad, w_pad = 0, 0
            _, _, h, w = imgs.size()
            if (h % mod_scale != 0):
                h_pad = (mod_scale - h % mod_scale)
            ...
            imgs = F.pad(imgs, (0, w_pad, 0, h_pad), 'reflect')
        print('imgs.shape 3333 f.pad after =======', imgs.shape)

        try:            
            with torch.no_grad():
                print('aaaaa')
                if self.tile_size > 0: # 무시하겠음 
                    output = self.tile_process(imgs)
                else:
                    #img = np.concatenate((img.cpu(),img.cpu()))
                    #img = torch.from_numpy(img).cuda()
                    #print('img shape ========', img.shape)
                    print('dddd imgs.shape ========', imgs.shape)
                    output_arr = self.srmodel(imgs.cuda())
            ...
            print('bbbb')
            if mod_scale is not None: # 일단 무시
                _, _, h, w = output_arr.size()
                output_arr = output_arr[:, :, 0:h - h_pad, 0:w - w_pad]
